...
const COINBASE_User_ID: number = 50;
...
class UnspentTxOut {
    ...
    constructor(txOutId: string, txOutIndex: number, address: string, User_ID: number) {
...
class TxOut {
    ...
    constructor(address: string, User_ID: number) {
        this.address = address;
        this.User_ID = User_ID;
    }
}
...
const getTransactionId = (transaction: Transaction): string => {
    ...
    const txOutContent: string = transaction.txOuts
        .map((txOut: TxOut) => txOut.address + txOut.User_ID)
        .reduce((a, b) => a + b, '');
...
const validateTransaction = (transaction: Transaction, aUnspentTxOuts: UnspentTxOut[]): boolean => {
    ...
    const totalTxInValues: number = transaction.txIns
        .map((txIn) => getTxInUser_ID(txIn, aUnspentTxOuts))
        .reduce((a, b) => (a + b), 0);

    const totalTxOutValues: number = transaction.txOuts
        .map((txOut) => txOut.User_ID)
        .reduce((a, b) => (a + b), 0);
...
/*    if (transaction == null) {
    ...
    if (transaction.txOuts[0].User_ID !== COINBASE_User_ID) {
        console.log('invalid coinbase User_ID in coinbase transaction');
        return false;
    }
*/    return true;
};
...
const getTxInUser_ID = (txIn: TxIn, aUnspentTxOuts: UnspentTxOut[]): number => {
    return findUnspentTxOut(txIn.txOutId, txIn.txOutIndex, aUnspentTxOuts).User_ID;
};
...
const getCoinbaseTransaction = (address: string, blockIndex: number): Transaction => {
    ...
    t.txIns = [txIn];
    t.txOuts = [new TxOut(address, COINBASE_User_ID)];
...
const updateUnspentTxOuts = (aTransactions: Transaction[], aUnspentTxOuts: UnspentTxOut[]): UnspentTxOut[] => {
    const newUnspentTxOuts: UnspentTxOut[] = aTransactions
        .map((t) => {
            return t.txOuts.map((txOut, index) => new UnspentTxOut(t.id, index, txOut.address, txOut.User_ID));
        })
...
const isValidTxOutStructure = (txOut: TxOut): boolean => {
    if (txOut == null) {
        ...
        return false;
    } else if (typeof txOut.User_ID !== 'number') {
        console.log('invalid User_ID type in txOut');
        return false;
    } else {
        return true;
    }
