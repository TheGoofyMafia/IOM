...
const generatenextBlockWithTransaction = (receiverAddress: string, User_ID: number) => {
    ...
    const coinbaseTx: Transaction = getCoinbaseTransaction(getPublicFromWallet(), getLatestBlock().index + 1);
    const tx: Transaction = createTransaction(receiverAddress, User_ID, getPrivateFromWallet(), getUnspentTxOuts(), getTransactionPool());
...
const sendTransaction = (address: string, User_ID: number): Transaction => {
    const tx: Transaction = createTransaction(address, User_ID, getPrivateFromWallet(), getUnspentTxOuts(), getTransactionPool());
