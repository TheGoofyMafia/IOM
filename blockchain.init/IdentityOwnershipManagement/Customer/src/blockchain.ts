...
const generatenextBlockWithTransaction = (receiverAddress: string, Shopkeeper_ID: number) => {
    if (!isValidAddress(receiverAddress)) {
        throw Error('invalid address');
    }
    if (typeof Shopkeeper_ID !== 'number') {
        throw Error('invalid Shopkeeper_ID');
    }
    const coinbaseTx: Transaction = getCoinbaseTransaction(getPublicFromWallet(), getLatestBlock().index + 1);
    const tx: Transaction = createTransaction(receiverAddress, Shopkeeper_ID, getPrivateFromWallet(), getUnspentTxOuts(), getTransactionPool());
...
const sendTransaction = (address: string, Shopkeeper_ID: number): Transaction => {
    const tx: Transaction = createTransaction(address, Shopkeeper_ID, getPrivateFromWallet(), getUnspentTxOuts(), getTransactionPool());
