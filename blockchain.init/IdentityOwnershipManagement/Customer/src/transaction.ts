...
const COINBASE_Shopkeeper_ID: number = 50;

class UnspentTxOut {
    public readonly txOutId: string;
    public readonly txOutIndex: number;
    public readonly address: string;
    public readonly Shopkeeper_ID: number;

    constructor(txOutId: string, txOutIndex: number, address: string, Shopkeeper_ID: number) {
...
class TxOut {
    ...
    constructor(address: string, Shopkeeper_ID: number) {
        this.address = address;
        this.Shopkeeper_ID = Shopkeeper_ID;
    }
}
...
const getTransactionId = (transaction: Transaction): string => {
    ...
    const txOutContent: string = transaction.txOuts
        .map((txOut: TxOut) => txOut.address + txOut.Shopkeeper_ID)
        .reduce((a, b) => a + b, '');
...
const validateTransaction = (transaction: Transaction, aUnspentTxOuts: UnspentTxOut[]): boolean => {
    ...
    const totalTxInValues: number = transaction.txIns
        .map((txIn) => getTxInShopkeeper_ID(txIn, aUnspentTxOuts))
        .reduce((a, b) => (a + b), 0);

    const totalTxOutValues: number = transaction.txOuts
        .map((txOut) => txOut.Shopkeeper_ID)
        .reduce((a, b) => (a + b), 0);
...
/*    if (transaction == null) {
    ...
    if (transaction.txOuts[0].Shopkeeper_ID !== COINBASE_Shopkeeper_ID) {
        console.log('invalid coinbase Shopkeeper_ID in coinbase transaction');
        return false;
    }
*/    return true;
};
...
const getTxInShopkeeper_ID = (txIn: TxIn, aUnspentTxOuts: UnspentTxOut[]): number => {
    return findUnspentTxOut(txIn.txOutId, txIn.txOutIndex, aUnspentTxOuts).Shopkeeper_ID;
};
...
const getCoinbaseTransaction = (address: string, blockIndex: number): Transaction => {
    ...
    t.txIns = [txIn];
    t.txOuts = [new TxOut(address, COINBASE_Shopkeeper_ID)];
...
const updateUnspentTxOuts = (aTransactions: Transaction[], aUnspentTxOuts: UnspentTxOut[]): UnspentTxOut[] => {
    const newUnspentTxOuts: UnspentTxOut[] = aTransactions
        .map((t) => {
            return t.txOuts.map((txOut, index) => new UnspentTxOut(t.id, index, txOut.address, txOut.Shopkeeper_ID));
        })
...
const isValidTxOutStructure = (txOut: TxOut): boolean => {
    if (txOut == null) {
        ...
        return false;
    } else if (typeof txOut.Shopkeeper_ID !== 'number') {
        console.log('invalid Shopkeeper_ID type in txOut');
        return false;
    } else {
        return true;
    }
